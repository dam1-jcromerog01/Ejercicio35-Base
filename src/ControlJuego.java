...
public class ControlJuego {
	...
	public void inicializarPartida(){
		...
		for (int i = 0; i < MINAS_INICIALES; i++)
		{
			...
			while (!placed)
			{
				...
				posY = (int)(Math.random()*tablero[0].length);
	...
	private int calculoMinasAdjuntas(int i, int j){
		...
		for (int fil = Math.max((i-1), 0) ; fil<=Math.min((i+1), (LADO_TABLERO-1)); fil++)
		{
			for (int col = Math.max((j-1), 0); col<=Math.min((j+1), (LADO_TABLERO-1)); col++)
			{
	...
	public boolean esFinJuego(){
		if (casillasAbiertas>=((Math.pow(LADO_TABLERO, 2)-MINAS_INICIALES)))
		{
			return true;
		}
		return false;
